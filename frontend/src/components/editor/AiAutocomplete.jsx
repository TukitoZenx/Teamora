...
export default function AiAutocomplete({ quillRef }) {
    ...
    const handleTextChange = (delta, oldDelta, source) => {
      ...
      debounceTimer.current = setTimeout(() => {
        ...
        if (range && range.length === 0) {
          // Only autocomplete if no text is selected
  ...
  return createPortal(
    ...
    >
      ...
      {!isLoading && suggestion && <span className="text-muted-foreground opacity-50">{suggestion}</span>}
    </div>,
    portalRoot
  )
}
