      ...
      <div className="flex-1 overflow-y-auto p-6">
        {currentItems.length === 0 ? (
          <div className="h-full flex flex-col items-center justify-center text-center p-6 bg-white dark:bg-slate-900 rounded-2xl border border-slate-200/50 dark:border-slate-800/80">
            ...
            <span className="text-xs text-slate-400 dark:text-slate-500 mt-1">Drag and drop or upload files to share them with your team.</span>
          </div>
        ) : (
            ...
            {currentItems.map((item) => (
              ...
              >
                ...
                <div className="min-w-0">
                  <h4 className="text-xs font-semibold text-slate-800 dark:text-slate-100 truncate block mt-3" title={item.name}>
                    {item.name}
                  </h4>
              ...
              <div className="w-64 border-l border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900 p-6 space-y-6 overflow-y-auto hidden md:block text-xs">
                <div>
                  ...
                  <div className="space-y-2 text-slate-600 dark:text-slate-300">
                    <p><span className="font-semibold">Type:</span> {previewFile.type}</p>
                    <p><span className="font-semibold">Size:</span> {formatSize(previewFile.size)}</p>
                    <p><span className="font-semibold">Uploaded by:</span> {previewFile.uploadedBy}</p>
                    <p><span className="font-semibold">Uploaded at:</span> {previewFile.uploadedAt}</p>
                  </div>
                </div>
                ...
                <div>
                  <h4 className="font-bold text-slate-400 uppercase tracking-wider text-[10px] mb-2">Version History</h4>
