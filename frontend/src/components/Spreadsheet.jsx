...
import { 
  ...
  BarChart3, ListFilter, Palette, LayoutGrid, Plus, Trash2, Edit3, 
  Lock, Unlock, ArrowLeftRight
} from 'lucide-react';
...
export default function Spreadsheet({
  ...
  const handleImportXLSX = (e) => {
    ...
    reader.onload = (evt) => {
      try {
        ...
        // populate into active sheet offset
  ...
  const handleExportXLSX = () => {
    try {
      ...
      const evaluatedGrid = visibleGridRows.map((row) => 
      ...
      <div className="flex-1 flex overflow-hidden">
          ...
          <table className="border-collapse w-max min-w-full">
            ...
            <tbody>
              {visibleGridRows.map((row, rIdx) => {
                ...
                return (
                  <tr key={rIdx} className={isFrozenRow ? 'sticky top-7 z-20 shadow-sm bg-indigo-50/50 dark:bg-indigo-950/20' : 'hover:bg-slate-50/20'}>
        ...
        {showCharts && (
          <div className="w-80 border-l border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-5 flex flex-col shrink-0 text-xs">
            ...
            {chartData.length === 0 ? (
              <p className="italic text-slate-450 text-center py-6">Enter numbers in the current column to display charts.</p>
            ) : chartType === 'bar' ? (
              <svg viewBox="0 0 200 120" className="w-full h-44 border border-slate-100 dark:border-slate-800 rounded-lg p-2 bg-slate-950">
                {chartData.map((d, i) => {
                  ...
                  return (
                    <g key={i}>
                      ...
                      <text x={x+5} y="105" fontSize="6" fill="#94a3b8" textAnchor="middle">{d.label}</text>
                      <text x={x+5} y={y-4} fontSize="6" fill="#ffffff" textAnchor="middle">{d.val}</text>
                    </g>
                  );
                })}
              </svg>
        ...
        {showPivotBuilder && (
          <div className="w-80 border-l border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-5 flex flex-col shrink-0 text-xs">
            <h4 className="font-bold text-[10px] text-slate-400 uppercase tracking-wider mb-4">Pivot Builder Summary</h4>
            <div className="bg-slate-50 dark:bg-slate-950 p-4 rounded-xl border border-slate-200/50">
              <span className="font-bold text-slate-700 dark:text-slate-300 block mb-2">Column A (Tags) & Column B (Sums)</span>
              <table className="w-full text-left text-[11px]">
                ...
                <tbody>
                  {(() => {
                    ...
                    if (entries.length === 0) {
                      return <tr><td colSpan={2} className="py-2 italic text-slate-400 text-center">Empty pivot range.</td></tr>;
                    }
          ...
          return (
            <div key={sIdx} className="relative group/tab flex items-center">
              ...
              <div className="hidden group-hover/tab:flex absolute bottom-full left-0 mb-1 bg-slate-950 text-white rounded-lg flex items-center p-1 gap-1.5 shadow-xl border border-white/10 z-50">
                <button onClick={() => handleRenameSheet(sheet.name)} title="Rename Sheet"><Edit3 className="w-3.5 h-3.5" /></button>
                <button onClick={() => handleDuplicateSheet(sheet)} title="Duplicate Sheet"><Copy className="w-3.5 h-3.5" /></button>
                <button onClick={() => handleDeleteSheet(sheet.name)} title="Delete Sheet" className="text-rose-400 hover:text-rose-500"><Trash2 className="w-3.5 h-3.5" /></button>
              </div>
            </div>
          );
        })}
        ...
        <div className="fixed inset-0 bg-slate-950/40 backdrop-blur-xs flex items-center justify-center z-50 p-4">
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 shadow-2xl max-w-sm w-full">
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block mb-1">Highlight if Value &gt;</label>
                <input
                  type="number"
                  value={ruleValue}
            ...
            <div className="flex justify-end gap-3 pt-6">
              <button onClick={() => setShowFormattingModal(false)} className="px-4 py-2 bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 text-slate-700 dark:text-slate-300 rounded-xl text-xs font-semibold">Cancel</button>
              <button onClick={addFormattingRule} className="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-xs font-semibold">Apply Rule</button>
            </div>
          </div>
        </div>
      )}
