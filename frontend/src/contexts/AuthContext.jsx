...
/** Test helper — clears the module-level /me promise cache between Vitest cases. */
export const __resetAuthBootstrapForTests = () => {
  ...
  const refreshUser = useCallback(
    async ({ useInitialCache = false } = {}) => {
      ...
      if (!sessionCheckedRef.current) {
        // what is does ? ans : it checks if the session has been checked or not, it mean s the first time the user is loading the app, so it will set the loading state to true
        setLoading(true)
      }
...
export function useSession() {
  ...
  if (!context) {
    // when it use ? ans : it is used to check if the context is not null or undefined
